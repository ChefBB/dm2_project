...
% The Random Forest classifier used balanced class weights, and was
% tuned using a randomized hyperparameter search.

% A \texttt{RandomizedSearchCV} procedure with 20 sampled configurations
% was employed, using 3-fold cross-validation and balanced accuracy as the
% optimization metric to account for class imbalance.
% Table~\ref{tab:rf_best_params} reports the best configuration found by
% the random search, along with some key metrics to measure performance.

% The parameters reflect a balanced model: enough trees and maximum
% depth for learning, while regularization via minimum samples and feature
% subsampling prevents overfitting.

% The model achieves an overall accuracy of 46\%, consistent with the
% previous model, while the macro-averaged F1-score is higher,
% indicating better handling of class imbalance.

% This is confirmed in the confusion matrix in Figure~\ref{fig:cm_shapelets}.
% The \textbf{Low} class in particular is considered more, relatively to
% the kNN classifiers. Errors for it and the \textbf{Medium High}
% are usually close to the correct class.

% The \textbf{Medium} and \textbf{High} classes have many instances
% in which they are misclassified as the other, as in both kNN classifiers,
% though performance for the latter class is a bit worse.

% Figure~\ref{fig:shapelets} shows the eight shapelets that provided
% the highest Information Gain.
% As can be seen in the legend, the more informative shapelets for the
% model are from the Low and Medium High classes, which are the least
% represented. The most informative Shapelet for the High class is the
% eleventh most informative overall, while for the Medium class it is
% the sixteenth.

% It is also interesting to note how the shapelets' lengths, shapes and
% position in their time series vary a lot.
