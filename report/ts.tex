...
task was also addressed using shapelet-based methods, with the
extracted shapelet features subsequently used to train a Random
Forest classifier.
...
The preprocessing pipeline first applies a logarithmic transformation
...
A \texttt{RandomShapeletTransform} is subsequently fitted on the
training data to extract a fixed set of discriminative subsequences,
which are used to transform both training and test sets into feature
representations.
These shapelet-based features are then concatenated with the
genre-related features to form a combined input space.
Finally, a standard feature-wise normalization is applied to the
combined feature matrix before training the downstream classifier.

The classifier was tuned using a randomized hyperparameter
search.
The search space included the number of trees, maximum tree depth,
minimum samples required for node splitting and leaf nodes, and the
number of features considered at each split.
A RandomizedSearchCV procedure with 20 sampled configurations was
employed, using 3-fold cross-validation and balanced accuracy as the
optimization metric to account for class imbalance.

\begin{table}[h]
    \centering
    \caption{Best Random Forest hyperparameters}
...
The most influential shapelet for the medium class is the 21st
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Most influential shapelet for the Low class}
        \label{fig:toplow}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
