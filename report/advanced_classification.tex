...
\subsubsection{Advanced classification conclusion}
...
The \texttt{LIME} visualization in Figure~\ref{fig:LIME_title} shows, how the most relevant 
...
used by \texttt{LIME} in the local surrogate model: for each class, \texttt{LIME} learns a simple linear classifier that separates that class 
...
according to the local surrogate, whereas bars aligned with \texttt{NOT tvSeries} capture conditions that would instead support other classes. 
...
stronger influence on the final decision. Overall, the figure conveys that specific combinations 
...
Figure~\ref{fig:LIME_rating} shows, the panel corresponding to the predicted class \([1, 5)\), the main contributing 
features have positive weights, meaning that their values for this instance resemble those 
commonly observed among titles in the lowest rating bin. For example, relatively 
...
the probability assigned to \([1, 5)\). In contrast, when looking at the panel for the 
true class \([6, 7)\), several of these same features tend to appear with negative contributions, 
indicating that, in the local surrogate model, they move the prediction \emph{away} from 
...
Taken together, the \texttt{LIME} explanation suggests that the misclassification is driven by 
