...
% The time series were first preprocessed using a logarithmic transformation (\texttt{log1p}) 
% to reduce skewness and stabilize variance. For the classical K-Nearest Neighbors (KNN) model 
% with Euclidean distance, we applied global standardization across all series to center and 
% scale the data. For the KNN model with Dynamic Time Warping (DTW), we applied per-series 
% amplitude scaling using \texttt{TimeSeriesScalerMeanVariance} to focus the distance on the 
% shape of each series rather than absolute magnitudes.
...
% The best number of neighbors was found to be 9. Using this configuration, the Euclidean KNN 
% achieved an accuracy of 0.48 and a macro F1-score of 0.33, showing better performance on the most 
% frequent classes but lower recall for underrepresented categories such 
% as ``Low'' and ``Medium High''. 

% We then applied KNN with DTW using the same number of neighbors and a 
% Sakoe--Chiba radius of 7, chosen considering the observed seasonality of approximately 
% 14 time steps. DTW achieved an accuracy of 0.43 and a macro F1-score of 0.35, improving 
% recall for the minority class ``Low'' while slightly decreasing performance on the other 
% classes.

% Overall, these results indicate that DTW can better handle small temporal shifts in 
% the series, whereas Euclidean distance benefits from global scaling and simplicity. 
% Categorical metadata, such as genre, was deliberately excluded to allow a direct 
% comparison of the two distance metrics on the raw time series.
...
$46\%$, with a macro-averaged F1-score of $0.37$, indicating a moderate classification 
performance in a multi-class and partially imbalanced setting. The confusion matrix reveals 
...
suggesting that 
movies with strong rating signals tend to share more consistent temporal patterns. 
In contrast, the \textit{Low} and \textit{Medium High} categories show substantially lower 
recall values and are frequently misclassified as neighboring classes, reflecting a smoother 
transition in temporal behavior between adjacent rating levels rather than clearly separable 
patterns. This behavior is consistent with the ordinal nature of the target variable, 
where errors tend to occur between semantically close categories rather than across 
distant ones.

achieved an accuracy of 0.46 and a macro F1-score of 0.37, ...
The corresponding confusion matrix, shown in Figure~\ref{fig:cm_knn}, highlights a 
relatively good performance for the \textit{High} and \textit{Medium} classes, 
while the \textit{Low} and \textit{Medium High} categories are more frequently 
confused with adjacent rating levels.
...
slightly lower accuracy of about $42\%$ and a macro F1-score of $0.35$, with performance trends similar to the Euclidean 
...
fact that rating categories are more strongly associated with overall trend and magnitude 
